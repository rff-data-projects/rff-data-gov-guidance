...
-   Save intermediate data to avoid repeated reads of large raw files. Save them in formats such as `.fst`, `.rds`, or `.parquet`, which are typically faster to read and write and more memory-efficient than plain text formats (e.g., `.csv`, `.txt`). See [Data & File Types](../data-management/file-formats.qmd) for more details on data formats. 
