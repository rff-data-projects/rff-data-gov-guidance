...
Profiling and benchmarking help pinpoint runtime and memory bottlenecks and quantify whether revisions improve performance. Options include:
...
-   Read only the data you need. Some read functions (e.g., `data.data::fread()` and `readr::read_csv()`) can scan the data structure and selectively load columns or rows without reading the entire dataset into memory.
