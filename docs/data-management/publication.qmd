...
The RFF github organization can fork the repository. This would basically make a copy of the repository at a certain point in your process. This would work well if you want to capture a certain snapshot of the model that's, say, associated with a certain publication. It can also be updated to sync with the latest version of the repository.
 
You can transfer ownership of the repository to the RFF org. All existing collaborators would still have write access (with no new permissions granted to anyone other than the organization owners). The repo URL would change. This is a good option for a more permanent re-homing with a neat URL.
