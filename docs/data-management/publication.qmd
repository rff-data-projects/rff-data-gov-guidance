...
2. Make the repository public once you're ready to publish if it isn't already.

3. Tag the publication version (e.g., v1.0-publication-name) as described [here](https://git-scm.com/book/en/v2/Git-Basics-Tagging). (Note: only admins can do this, request admin privileges if needed.)

If your repository is under a personal GitHub account:

2. Decide between **forking** or **transferring** the repo to the RFF GitHub organization (see explanation of these options above).

3. Contact the RFF GitHub organization admins at [DGWG@rff.org](mailto:dgwg@rff.org) to initiate the fork or transfer and to add a tag (*note: tags do not transfer automatically when a repo is forked*).
