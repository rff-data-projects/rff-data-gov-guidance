...
Plain text formats like CSV (.csv) are preferred for their transparency, interoperability, and long-term accessibility. Excel (`.xlsx`) and Stata (`.dta`) are <span style="color:red;">proprietary</span> and should generally be avoided—except when essential features (like Stata variable labels) are required.

**Other formats may be appropriate when needed.**
The preferred type of open source format would depend on the project team's preference for software, retaining data types, etc.
