...
**Use open-source formats whenever possible.**
Plain text formats like CSV (.csv) are preferred for their transparency, interoperability, and long-term accessibility.

**Other formats may be appropriate when needed.**
Formats such as Feather, Parquet, or RDS can optimize performance (e.g., faster read/write) or preserve data types, but may reduce compatibility across tools.

**Avoid proprietary formats unless necessary.**
Excel (`.xlsx`) and Stata (`.dta`) are <span style="color:red;">proprietary</span> and should generally be avoided—except when essential features (like Stata variable labels) are required.
...
* **Memory efficiency** Efficient data type choices can improve performance for large datasets. For example, integer values (such as number of people) stored as float values incurs unnecessary memory costs.
...
**Be cautious when reading and writing CSVs or other plain-text formats. Understand that data types will be inferred, not preserved, and this may introduce rounding or formatting errors.**

*All values are saved as **character strings** in text-based file formats* — there is no embedded information about whether something is an integer, float, etc. This has several implications:
...
```
DRAFT
POTENTIAL SOLUTIONS TO LOSS OF DATA TYPE METADATA IN CSVS

type guessing on import:
- Explicitly specify column types when importing data (e.g., using read_csv(..., col_types=...) in R or dtype=... in pandas).
- Validate types after import.

precision loss on export & no built-in support for special types:
- Use formats that preserve numeric precision (e.g., binary formats like .rds, .feather, .parquet) during analysis.
- Export summaries/outputs to CSV only WHEN NEEDED? WHEN PUBLISHING / SHARING?
- Clearly document data types in metadata or codebook when using and sharing CSVs.
```
