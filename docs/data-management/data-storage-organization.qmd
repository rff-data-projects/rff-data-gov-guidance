...
The best format to use will vary depending on the characteristics of the data being stored, but some common examples of proprietary file types to avoid are Excel (.xlsx) and Stata (.dta) (see Note below). Instead, use open-source plain text formats such as comma-separated values (.csv).
...
|--------|-----------|----------------------------|--------------------------------|-------------------|
| ***Recommended:*** **comma-separated values** | .csv | Open-source | No | Structured |
| Tab-separated values | .tsv | Open-source | No | Structured |
...
| DTA | .dta | Proprietary | Yes | Structured |
...
- **Feather (.feather)**, a fast, lightweight binary columnar data format used for data exchange between data analysis languages like R and Python. Optimized for performance.
