...
* RFF OneDrive project folders start with 5 TB of storage, which can be increased if needed (not for external users’ accounts).
* External collaborators added as guests are subject to your account’s storage limits.
* Not suitable for large files: syncing can fail with files over 10 GB.
* It is not recommended to load data directly from OneDrive into scripts—this can cause sync issues and delays. Instead, copy data to the project’s `L:/` drive folder, which is the authoritative storage location.
* RFF lab computers can only access OneDrive through a web browser. Use the `L:/` drive to access data on lab machines.
