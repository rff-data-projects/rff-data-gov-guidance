...
* A local clone of a repository should generally ONLY have a single user. 
  * This is why in section (<INSERT LINK TO DM SECTION>) we recommend that each user works from a repository clone stored in their own individual subfolder within the L:/ drive project folder.  
...
* __Project__:  If your team has a project that requires making project-specific changes, it _could_ be a good idea to create a project branch, with no intention to merge the project branch into `main`.  :warning:However, for features that would be useful to have in the `main` branch, it is best to implement those in the main branch or in a separate branch (to be merged with main).  It is difficult to selectively merge changes from the project branch into the `main` branch.
