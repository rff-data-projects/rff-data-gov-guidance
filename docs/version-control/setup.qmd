...
It is necessary to install git onto any computer that you may wish to interact with local repositories.
There are many ways to interact with git, including several applications with graphical user interfaces, which may help to visualize what is happening.
Normally, IT has installed Git on RFF computers. If not, please send a request to [IThelp@rff.org](mailto:ithelp@rff.org) to request a git install.
...
This is the industry standard, so it is what we suggest for new git users.
...
Say there is an existing repository on GitHub that you would like to download and modify.
The process for creating a local copy on your computer is called [cloning](./terminology.qmd#clone).
...
* `printf "Host github.com\n  Hostname ssh.github.com\n  Port 443" >> ~/.ssh/config` - This command writes 3 line to the newly created config file.
