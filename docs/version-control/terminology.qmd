...
#### Organization
A github account associated with a group.  Users can be part of multiple organizations.  I.e. [e4st-dev](https://github.com/e4st-dev)
...
* __Remote Repository__: A copy of the repository stored on the cloud, for our purposes in Github.  I.e. [E4ST.jl](https://github.com/e4st-dev/E4ST.jl)
...
#### Relative File Paths
A path telling how to get from one file to another.  For example, say I have a script `process.jl` and I need to reference a data file, `data.csv`, but they are in different folders:
...
From within the `process.jl`, the relative filepath to `data.csv` would be `../data/data.csv`.  The prepending `../` represents "backing up" one folder level.  In this example, `../` backs up from the `project/scripts/` to `project/`.
